...
?>
    ...
    <form action='../../controller/logout.php' method='get' ><input type='submit' value='logout'></form>
            ...
            <div class ="Page">
              ...
              <h2 align="center">Liste Des Ecoles Universitaires </h2></br>
...
?>
    ...
    <tr>
    <th  id=<?= $f1->id_categorie ?>scope="row">
        <?php         
        if (($f1->actif)){
        echo '<a class="lien" href="../../sous-sites/'.$f1->id_categorie.'/'.$i.'/index.php">
        '.$f1->nom.'</a></th>';      
        }else echo $f1->nom.'</th>';
        ?></th>        <td id=<?= $f1->id_categorie ?>><?= $f1->domaine ?></td>
        <td id=<?= $f1->id_categorie ?>><?= $f1->wilaya ?></td>
        <td id=<?= $f1->id_categorie ?>><?= $f1->commune ?></td>
        <td id=<?= $f1->id_categorie ?>><?= $f1->adresse ?></td>
        <td id=<?= $f1->id_categorie ?>><?= $f1->tel ?></td>
        <td id=<?= $f1->id_categorie ?>><?= $f1->fax ?></td>
...
<?php
           ...
           if(isset($_SESSION['user']) &&  !($_SESSION['admin'])){             
               ...
               echo "<td class=\" .$f1->id_categorie.\"><a class=\"lien\" href=\"../administration/commentaire.php?id_cat=$f1->id_categorie\">Commenter</a></td>";
...
?>
        ...
        <script src="../public/js/gestion_commentaire.js"> </script>
