...
?>
    ...
    <form action='../../controller/logout.php' method='get' ><input type='submit' value='logout'></form>
        ...
        <div class="vertical-menu">
              ...
              <h2 align="center">Liste Des Ecoles Professionnelles </h2></br></br>
...
<button type="submit" class="filtre-btn"  id="filtre-btn">filtrer</button>
 ...
 <table id ="univ-table" border="1px" align="center" >
    ...
    foreach ($fs2 as $f2) : ?>
    <tr>
        <th  id=<?= $f2->id_categorie ?>scope="row">
        <?php         
        if (($f2->actif)){
        echo '<a class="lien" href="../../sous-sites/'.$f2->id_categorie.'/'.$i.'/index.php">
        '.$f2->nom.'</a></th>';      
        }else echo $f2->nom.'</th>';
        ?></th>
        <td id=<?= $f2->id_categorie ?>><?= $f2->domaine ?></td>
        <td id=<?= $f2->id_categorie ?>><?= $f2->wilaya ?></td>
        <td id=<?= $f2->id_categorie ?>><?= $f2->commune ?></td>
        <td id=<?= $f2->id_categorie ?>><?= $f2->adresse ?></td>
        <td id=<?= $f2->id_categorie ?>><?= $f2->tel ?></td>
        <td id=<?= $f2->id_categorie ?>><?= $f2->fax ?></td>
...
if (isset($_SESSION['user']) && !($_SESSION['admin'])) {
    ...
    echo "<td class=\" .$f2->id_categorie.\"><a class=\"lien\" href=\"../administration/commentaire.php?id_cat=$f2->id_categorie\">Commenter</a></td>";
