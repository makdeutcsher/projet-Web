...
?>
    ...
    <form action='../../controller/logout.php' method='get' ><input type='submit' value='logout'></form>
        ...
        <div class="vertical-menu">
              ...
              <h2 align="center">Liste Des Ecoles Moyennes </h2></br></br>
...
$i=8;
    foreach ($fs4 as $f4) : ?>
        <tr>
        <th  id=<?= $f4->id_categorie ?>scope="row">
        <?php         
        if (($f4->actif)){
        echo '<a class="lien" href="../../sous-sites/'.$f4->id_categorie.'/'.$i.'/index.php">
        '.$f4->nom.'</a></th>';      
        }else echo $f4->nom.'</th>';
        ?></th>            <td id=<?= $f4->id_categorie ?>><?= $f4->domaine ?></td>
            <td id=<?= $f4->id_categorie ?>><?= $f4->wilaya ?></td>
            <td id=<?= $f4->id_categorie ?>><?= $f4->commune ?></td>
            <td id=<?= $f4->id_categorie ?>><?= $f4->adresse ?></td>
            <td id=<?= $f4->id_categorie ?>><?= $f4->tel ?></td>
            <td id=<?= $f4->id_categorie ?>><?= $f4->fax ?></td>
            <td >Moyennes</td>
...
<?php
           ...
           if(isset($_SESSION['user']) &&  !($_SESSION['admin'])){             
               ...
               echo "<td class=\" .$f4->id_categorie.\"><a class=\"lien\" href=\"../administration/commentaire.php?id_cat=$f4->id_categorie\">Commenter</a></td>";
